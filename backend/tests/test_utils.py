...
class TestSortPromptsByDate:
    ...
    async def test_sort_prompts_descending(self):
    ...
    async def test_sort_prompts_ascending(self):
    ...
    async def test_sort_single_prompt(self):
    ...
    async def test_sort_prompts_with_same_timestamp(self):
    ...
    async def test_sort_prompts_with_none_created_at(self):
    ...
    async def test_sort_prompts_default_descending(self):
    ...
    async def test_sort_prompts_large_list(self):
    ...
    async def test_sort_with_identical_timestamps_different_objects(self):
...
class TestFilterPromptsByCollection:
    ...
    async def test_filter_prompts_by_collection_id(self):
    ...
    async def test_filter_prompts_no_match(self):
    ...
    async def test_filter_prompts_preserves_order(self):
    ...
    async def test_filter_prompts_case_sensitive_collection_id(self):
    ...
    async def test_filter_prompts_with_empty_collection_id(self):
    ...
    async def test_filter_prompts_large_list(self):
    ...
    async def test_filter_collection_id_case_sensitive_with_none(self):
...
class TestSearchPrompts:
    ...
    async def test_search_by_title(self):
        ...
        results = await search_prompts(prompts, "world")
    ...
    async def test_search_by_description(self):
        ...
        results = await search_prompts(prompts, "python")
        assert len(results) == 2
    ...
    async def test_search_empty_query(self):
        ...
        results = await search_prompts(prompts, "")
        assert len(results) == 2
    ...
    async def test_search_no_results(self):
        ...
        results = await search_prompts(prompts, "java")
        assert len(results) == 1
        assert results[0].title == "JavaScript"

    async def test_search_with_none_description(self):
    ...
    async def test_search_substring_match(self):
    ...
    async def test_search_with_special_characters(self):
        ...
        results = await search_prompts(prompts, "3.11")
        assert len(results) == 1
    ...
    async def test_search_with_unicode_characters(self):
        ...
        results = await search_prompts(prompts, "é")
        assert len(results) == 1
        assert results[0].title == "Café"

        results = await search_prompts(prompts, "ï")
        assert len(results) == 1
    ...
    async def test_search_with_whitespace_query(self):
        ...
        results = await search_prompts(prompts, "Hello World")
        assert len(results) == 1
        assert results[0].title == "Hello World"

    async def test_search_prompts_with_regex_special_chars(self):
        ...
        results = await search_prompts(prompts, "File[1].txt")
        assert len(results) == 1
        assert results[0].title == "File[1].txt"

    async def test_search_prompts_with_very_long_query(self):
        ...
        results = await search_prompts(prompts, long_query)
        assert len(results) == 0

        results = await search_prompts(prompts, "a" * 10)
        assert len(results) == 1

    async def test_search_prompts_with_very_long_title(self):
        ...
        results = await search_prompts(prompts, "a" * 50)
        assert len(results) == 1
        assert results[0].title == long_title

    async def test_search_case_insensitive_with_mixed_case(self):
    ...
    async def test_search_with_whitespace_only_query(self):
...
class TestValidatePromptContent:
    ...
    async def test_invalid_empty_content(self):
        """Empty string returns False."""
        assert validate_prompt_content("") == False

    async def test_invalid_whitespace_only(self):
    ...
    async def test_validation_with_newlines_and_tabs(self):
...
class TestExtractVariables:
    ...
    async def test_extract_no_variables(self):
    ...
    async def test_extract_variables_with_underscores(self):
    ...
    async def test_extract_variables_with_numbers(self):
    ...
    async def test_extract_malformed_variables(self):
    ...
    async def test_extract_variables_case_sensitive(self):
        """Variable names are case-sensitive; {{Name}} and {{name}} are distinct."""
        content = "{{Name}} and {{name}}"
        variables = extract_variables(content)
        assert variables == ["Name", "name"]

    async def test_extract_variables_with_nested_braces(self):
    ...
    async def test_extract_variables_with_dashes(self):
    ...
    async def test_extract_variables_with_dots(self):
    ...
    async def test_extract_variables_edge_cases(self):
    ...
    async def test_extract_variables_with_many_repeats(self):
    ...
    async def test_extract_variables_with_very_long_content(self):
    ...
    async def test_extract_variables_with_unicode_in_names(self):
    ...
    async def test_extract_variables_with_different_whitespace(self):
...
class TestIntegration:
    ...
    async def test_integration_sort_filter_search(self):
        ...
        searched = await search_prompts(filtered, "python")
    ...
    async def test_integration_validate_and_extract(self):
    ...
    async def test_edge_case_empty_string_vs_none(self):
...
class TestFuzzySearchEdgeCases:
    ...
    async def test_fuzzy_single_char_no_match_returns_empty(self):
        ...
        results = await search_prompts(prompts, "a", fuzzy=True)
        assert results == []

    async def test_fuzzy_single_char_exact_match_returns_result(self):
    ...
    async def test_fuzzy_results_sorted_best_match_first(self):
        ...
        results = await search_prompts(prompts, "match", fuzzy=True)
        assert len(results) >= 1
        if len(results) == 2:
            assert results[0].title == "match is the very first word"

    async def test_exact_search_title_match_sufficient_for_inclusion(self):
        ...
        results = await search_prompts(prompts, "python", fuzzy=False)
        assert len(results) == 1

    async def test_exact_search_no_match_returns_empty(self):
    ...
    async def test_exact_search_is_case_insensitive(self):
        """Exact search matches regardless of case differences."""
        prompts = [Prompt(title="PYTHON Guide", content="content")]
        results = await search_prompts(prompts, "python", fuzzy=False)
        assert len(results) == 1

    async def test_whitespace_only_query_returns_all_prompts(self):
        """A whitespace-only query string returns all prompts unchanged."""
        prompts = [
            Prompt(title="Prompt A", content="content"),
            Prompt(title="Prompt B", content="content"),
        ]
        results = await search_prompts(prompts, "   ", fuzzy=False)
        assert len(results) == 2

    async def test_tab_only_query_returns_all_prompts(self):
    ...
    async def test_validate_prompt_content_9_chars_fails(self):
        """validate_prompt_content returns False for 9 stripped chars and True for 10."""
        assert validate_prompt_content("123456789") == False
        assert validate_prompt_content("1234567890") == True

    async def test_validate_prompt_content_9_chars_with_surrounding_whitespace(self):
        """Surrounding whitespace is stripped before the length check; 9 stripped chars fails."""
        assert validate_prompt_content("  123456789  ") == False

    async def test_search_field_tags_returns_only_matching_prompts(self):
        ...
        results = await search_prompts(prompts, "python", fuzzy=False, search_field="tags")
        assert len(results) == 1
        assert "python" in results[0].tags

    async def test_exact_search_description_only_match(self):
